...
@section('content')
			...
			<!-- Bug report form -->
			<form action="{{url('admin/menu-forth-sub-category-store')}}" method="post" role="form">
				@csrf
	            <div class="panel panel-default">
	                <div class="panel-heading"><h6 class="panel-title"><i class="icon-bug"></i> Add Forth Sub Category</h6></div>
						...
						<div class="form-group">
							<div class="row">
								<div class="col-md-6">
	                                ...
	                                <input type="text" name="forth_sub_category" class="form-control" placeholder="Wedding & Party" required="required">
								</div>
								<div class="col-md-6">
			...
			<!-- /bug report form -->
			  <div class="panel panel-default">
			                <div class="panel-heading"><h6 class="panel-title"><i class="icon-table"></i>Forth Sub Category List</h6></div>
			                <div class="datatable">
				                <table class="table">
				                    <thead>
				                        <tr>
				                    ...
				                    $i=0;
				                    @endphp
				                    @foreach($MenuForthSubCategory as $forth_cat)
				                    <?php $i++;?>
				                        <tr>
				                            <td>{{$i}}</td>
				                            <td>
				                            	<?php
				                            	 $top_cat = DB::table('menu_categories')->where('id', $forth_cat->category_id)->first();
				                            ...
				                            <td>
				                            	<?php
				                            	 $sub_cat = DB::table('menu_sub_categories')->where('id', $forth_cat->sub_category_id)->first();
				                            	 if($sub_cat){
				                            ...
				                            <td>
				                            	<?php
				                            	 $tri_cat = DB::table('menu_tri_sub_categories')->where('id', $forth_cat->tri_sub_category_id)->first();
				                            	 if($tri_cat){
				                            	 	echo $tri_cat->tri_sub_category_name;
				                            	 }
				                            	 
				                            	?>

				                            </td>
				                            <td>{{$forth_cat->forth_sub_category_name}}</td>
				                            <td>
				                            <a href="{{url('admin/menu-forth-sub-category-edit/'.$top_cat->id.'/'.$sub_cat->id.'/'.$tri_cat->id.'/'.$forth_cat->id)}}" class="btn btn-icon btn-info"><i class="icon-pencil3"></i></a>
				                            <a onclick="return confirm('Are you sure to delete this item?')" href="{{url('admin/menu-forth-sub-category-delete',$forth_cat->id)}}" class="btn btn-icon btn-danger"><i class="icon-remove2"></i></a>
				                            </td> 
				                        </tr>
				                     @endforeach  
				                    </tbody>
